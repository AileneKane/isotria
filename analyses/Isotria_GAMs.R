...
ClimDat$month<-substr(ClimDat$DATE,6,7)
...
print(summary(gam0b))
plot(gam0b,main="Dormant (prev. fl/fr),nU=20,strt=5")
# and now, fit a GAM for dormant(prev veg) plants
gam0b = gam(logit(Stage_curdv) ~ s(lags, by=tcovar, bs="cs"),
            data=dat1, method="GCV.Cp",gamma=1.2, family = gaussian) 
print(summary(gam0b))
plot(gam0b,main="Dormant (prev. veg),nU=20,strt=5")
# and now, fit a GAM for vegetative plants
gam0b = gam(logit(Stage_curv) ~ s(lags, by=tcovar, bs="cs"),
            data=dat1, method="GCV.Cp",gamma=1.2, family = gaussian) 
print(summary(gam0b))
plot(gam0b,main="Vegetative,nU=20,strt=5")
# and now, fit a GAM for flowering plants
gam0b = gam(logit(Stage_curfl) ~ s(lags, by=tcovar, bs="cs"),
            data=dat1, method="GCV.Cp",gamma=1.2, family = gaussian) 
print(summary(gam0b))
plot(gam0b,main="Flowering,nU=20,strt=5")
# and now, fit a GAM for fruiting plants
gam0b = gam(logit(Stage_curfr) ~ s(lags, by=tcovar, bs="cs"),
            data=dat1, method="GCV.Cp",gamma=1.2, family = gaussian) 
print(summary(gam0b))
plot(gam0b,main="Fruiting,nU=20,strt=5")
