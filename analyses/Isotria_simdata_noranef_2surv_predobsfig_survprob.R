...
#Modified dec 14, 2017 to run through different survival probabilities, keeping flowering and dormancy constant
#different survival probs=c(0.7,0.75, 0.8,0.85, 0.9,0.95)
...
#dufvals<-c(0.05,0.25,0.45,0.65,0.85)
#fuvvals<-c(0.2, 0.5, 0.8)
#duvvals<-c(0.05,0.25,0.45,0.65,0.85)
...
path="C:/Users/aettinger/Documents/isotria/simdat_modsamples/sprobs"
...
pathsum="C:/Users/aettinger/Documents/isotria/simdat_modsums/sprobs"
...
#Now make a figure of estimate vs true of all these simulations:
...
row.names(ms4.rf)<-ms4.rf[,1]
ms4.rf<-ms4.rf[,-1]
sV.df[j,1]<-as.numeric(substr(namesum,11,14))
sV.df[j,2]<-ms4.rf$mean[1]
sV.df[j,3]<-ms4.rf[1,grep("2.5",colnames(ms4.rf))]
sV.df[j,4]<-ms4.rf[1,grep("97.5",colnames(ms4.rf))]

sF.df[j,1]<-as.numeric(substr(namesum,19,22))
sF.df[j,2]<-ms4.rf$mean[2]
sF.df[j,3]<-ms4.rf[2,grep("2.5",colnames(ms4.rf))]
sF.df[j,4]<-ms4.rf[2,grep("97.5",colnames(ms4.rf))]
fV.df[j,1]<-fV
...
#UV stage
plot(dUV.df[,1],dUV.df[,2], pch=21, bg="gray", xlim=c(0,1), ylim=c(0,1), xlab="True value", ylab="Estimate", main="dUV (2 Surv. Mod)")
abline(a=0, b=1, lty=1 )
for(i in 1:dim(dUV.df)[1]){
  arrows(dUV.df[i,1],dUV.df[i,3],dUV.df[i,1],dUV.df[i,4], length=0.01, angle=90, code=3)
}
points(dUV.df[,1],dUV.df[,2], pch=21, bg="gray")
#UF stage
dUF.df[,1]<-as.numeric(dUF.df[,1])
dUF.df[,2]<-as.numeric(dUF.df[,2])

plot(dUF.df[,1],dUF.df[,2], pch=21, bg="gray", xlim=c(0,1), ylim=c(0,1), xlab="True value", ylab="Estimate", main="dUF (2 Surv. Mod)")
abline(a=0, b=1, lty=1 )
for(i in 1:dim(dUF.df)[1]){
  arrows(dUF.df[i,1],dUF.df[i,3],dUF.df[i,1],dUF.df[i,4], length=0.01, angle=90, code=3)
}
points(dUF.df[,1],dUF.df[,2], pch=21, bg="gray")
