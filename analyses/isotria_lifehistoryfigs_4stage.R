...
library(popbio)
mod.samples<-read.csv("msmod_samples_4stage.csv", header=T)
# vital rates for group X prior to clearing
sV_Xpre<-mod.samples[,which(colnames(mod.samples)=="sV0.1.")]#if not read in, colname=psiA0[1]
sR_Xpre<-mod.samples[,which(colnames(mod.samples)=="sF0.1.")]#if not read in, colname=psiB0[1]
pdormV_Xpre<-mod.samples[,which(colnames(mod.samples)=="dV0.1.")]#if not read in, colname=pA0[1]
pdormR_Xpre<-mod.samples[,which(colnames(mod.samples)=="dF0.1.")]#if not read in, colname=pB0[1]
pflowV_Xpre<- mod.samples[,which(colnames(mod.samples)=="fV0.1.")]#if not read in, colname=phiA0[1]
pflowR_Xpre<-mod.samples[,which(colnames(mod.samples)=="fR0.1.")]#if not read in, colname=phiB0[1]

# vital rates for group Y prior to clearing
sV_Ypre<-mod.samples[,which(colnames(mod.samples)=="sV0.2.")]#if not read in, colname=psiA0[2]
sR_Ypre<-mod.samples[,which(colnames(mod.samples)=="sR0.2.")]#if not read in, colname=psiB0[2]
pdormV_Ypre<-mod.samples[,which(colnames(mod.samples)=="dV0.2.")]#if not read in, colname=pA0[2]
pdormR_Ypre<-mod.samples[,which(colnames(mod.samples)=="dR0.2.")]#if not read in, colname=pB0[2]
pflowV_Ypre<- mod.samples[,which(colnames(mod.samples)=="fV0.2.")]#if not read in, colname=phiA0[2]
pflowR_Ypre<-mod.samples[,which(colnames(mod.samples)=="fR0.2.")]#if not read in, colname=phiB0[2]

# vital rates for group X after clearing
sV_Xpost<-mod.samples[,which(colnames(mod.samples)=="sV1.1.")]#if not read in, colname=psiA1[1]
sR_Xpost<-mod.samples[,which(colnames(mod.samples)=="sR1.1.")]#if not read in, colname=psiB1[1]
pdormV_Xpost<-mod.samples[,which(colnames(mod.samples)=="dV1.1.")]#if not read in, colname=pA1[1]
pdormR_Xpost<-mod.samples[,which(colnames(mod.samples)=="dR1.1.")]#if not read in, colname=pB1[1]
pflowV_Xpost<- mod.samples[,which(colnames(mod.samples)=="fV1.1.")]#if not read in, colname=phiA1[1]
pflowR_Xpost<-mod.samples[,which(colnames(mod.samples)=="fR1.1.")]#if not read in, colname=phiB1[1]

# vital rates for group Y after clearing
sV_Ypost<-mod.samples[,which(colnames(mod.samples)=="sV1.2.")]#if not read in, colname=psiA1[2]
sR_Ypost<-mod.samples[,which(colnames(mod.samples)=="sR1.2.")]#if not read in, colname=psiB1[2]
pdormV_Ypost<-mod.samples[,which(colnames(mod.samples)=="dV1.2.")]#if not read in, colname=pA1[2]
pdormR_Ypost<-mod.samples[,which(colnames(mod.samples)=="dR1.2.")]#if not read in, colname=pB1[2]
pflowV_Ypost<- mod.samples[,which(colnames(mod.samples)=="fV1.2.")]#if not read in, colname=phiA1[2]
pflowR_Ypost<-mod.samples[,which(colnames(mod.samples)=="vR1.2.")]#if not read in, colname=phiB1[2]
...
ms3a<-read.csv("isotria4stagemodsum.csv", header=T)
rownames(ms3a)<-ms3a[,1]
surv_veg<-as.data.frame(rbind(ms3a$mean[grep("sV0",substr(rownames(ms3a),1,3))],ms3a$mean[grep("sV1",substr(rownames(ms3a),1,3))]))
surv_rep<-as.data.frame(rbind(ms3a$mean[grep("sF0",substr(rownames(ms3a),1,3))],ms3a$mean[grep("sF1",substr(rownames(ms3a),1,3))]))
dor_veg<-as.data.frame(rbind(ms3a$mean[grep("dV0",substr(rownames(ms3a),1,3))],ms3a$mean[grep("dV1",substr(rownames(ms3a),1,3))]))
dor_rep<-as.data.frame(rbind(ms3a$mean[grep("dF0",substr(rownames(ms3a),1,3))],ms3a$mean[grep("dF1",substr(rownames(ms3a),1,3))]))
pflow_veg<-as.data.frame(rbind(ms3a$mean[grep("fV0",substr(rownames(ms3a),1,5))],ms3a$mean[grep("fV1",substr(rownames(ms3a),1,5))]))
pflow_rep<-as.data.frame(rbind(ms3a$mean[grep("fF0",substr(rownames(ms3a),1,5))],ms3a$mean[grep("fF1",substr(rownames(ms3a),1,5))]))

colnames(surv_veg)<-c("control","logged")
colnames(surv_rep)<-c("control","logged")
colnames(dor_veg)<-c("control","logged")
colnames(dor_rep)<-c("control","logged")
colnames(pflow_veg)<-c("control","logged")
colnames(pflow_rep)<-c("control","logged")
...
plot(x,c(dor_veg$control,dor_veg$logged), pch=21, bg="black", ylim=c(0,1), ylab="Dormancy", xaxt="n", cex=1.5, xlab="", xlim=c(0.75,4.25), cex.lab=1.5, las=1, cex.axis=1.3)
lines(x[1:2],c(dor_veg$control), lty=1)
lines(x[3:4],c(dor_veg$logged), lty=3)
abline(v=2.5,lty=1, lwd=2)
abline(v=1.5,lty=2,col="gray", lwd=2)
abline(v=3.5,lty=2,col="gray", lwd=2)
arrows(xerror,1-emer_veg_q2.5,xerror,1-emer_veg_q97.5, code=0,angle=90, length=0.1)
points(x,c(dor_veg$control,dor_veg$logged),pch=21, bg=c("black","black","white","white"), cex=1.5)
arrows(x2error,1-emer_rep_q2.5,x2error,1-emer_rep_q97.5, code=0,angle=90, length=0.1)
lines(x2[1:2],c(dor_rep$control), lty=1)
lines(x2[3:4],c(dor_rep$logged), lty=3)
points(x2,c(dor_rep$control,dor_rep$logged),pch=21, bg=c("black","black","white","white"), cex=1.5)
#cbind(rownames(ms3a[41:48,]),1-ms3a$mean[41:48],1-ms3a$X2.5[41:48],1-ms3a$X97.5.[41:48])#check error bars
#transition
plot(x,c(pflow_veg$control,pflow_veg$logged), pch=21, bg=c("black","black","white","white"), ylim=c(0,1), ylab="Flowering", xaxt="n", cex=1.6, xlab="", xlim=c(0.75,4.25), cex.lab=1.5, las=1, cex.axis=1.3)
lines(x[1:2],c(pflow_veg$control), lty=1)
lines(x[3:4],c(pflow_veg$logged), lty=3)
abline(v=2.5,lty=1, lwd=2)
abline(v=1.5,lty=2,col="gray", lwd=2)
abline(v=3.5,lty=2,col="gray", lwd=2)
arrows(xerror,trans_vr_q2.5,xerror,trans_vr_q97.5, code=0,angle=90, length=0.1)
points(x,c(pflow_veg$control,pflow_veg$logged),pch=21, bg=c("black","black","white","white"), cex=1.5)
arrows(x2error,trans_rv_q2.5,x2error,trans_rv_q97.5, code=0,angle=90, length=0.1)
lines(x2[1:2],c(pflow_rep$control), lty=1)
lines(x2[3:4],c(pflow_rep$logged), lty=3)
points(x2,c(pflow_rep$control,pflow_rep$logged),pch=21, bg=c("black","black","white","white"), cex=1.5)
